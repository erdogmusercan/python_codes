...
def get_network(sub_network):
	if loc == "1":
		network = open ("Esenyurt Route Table List.txt","w")
		loca = "DC90ESY01"
		supernet = open("Esenyurt_Subnets_Supernets.txt","r")
	elif loc == "2":
		network = open ("Tuzla Route Table List.txt","w")
		loca = "DC90TZL01"
		supernet = open("Tuzla_Subnets_Supernets.txt","r")
	elif loc == "3":
		network = open ("Gaziemir Route Table List.txt","w")
		loca = "DC90GZM01"
		supernet = open("Gaziemir_Subnets_Supernets.txt","r")
	elif loc == "4":
		network = open ("Pursaklar Route Table List.txt","w")
		loca = "DC90PSK01"
		supernet = open("Pursaklar_Subnets_Supernets.txt","r")
	...
	client = ssh.invoke_shell()
	client.send("Term len 0\n")
	client.send("show ip route vrf all | ex connected\n")
	time.sleep(1)
	output = client.recv(999999)
	output = output.decode().split('\r\n')
	tmp = []
	for item in output:
		if 'ubest' in item:
			if not '0.0.0.0/0' in item:
				tmp.append(item.split(",")[0] + "\n")
	network.writelines(tmp)
	network.close()
	tmp = [ x.strip('\n') for x in tmp]
	supper = json.loads(supernet.read())
	for item in supper:
		if not item in tmp:
			for subber in supper[item]:
				if subber.endswith("/{}".format(sub_network)):
					for eleman in tmp:
						if ipaddress.IPv4Network(subber).overlaps(ipaddress.IPv4Network(eleman)):
							break
						else:
							continue
...
sub_network = input("""
*** Bulmak istediginiz network genişliğini "/" olmadan sadece sayı olarak giriniz
